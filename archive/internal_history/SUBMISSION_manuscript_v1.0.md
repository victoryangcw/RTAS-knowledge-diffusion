...
**Projects.** 3,714 undergraduate innovation projects approved in 2020–2024 at one large comprehensive Chinese university (university-level 1,375, 37.0%; provincial 1,657, 44.6%; national 682, 18.4%), each with title (on average 92% Chinese characters), level, year, college (40), and advisor(s). Multi-advisor fields were split using the same delimiter rules as in advisor-covariate construction, yielding 1,834 individual advisors and 4,244 advisor–project links (528 projects were co-supervised).

**Articles.** 56,901 articles (type=article, 2020–2024) retrieved from OpenAlex for institution I37461747 via cursor pagination, with predominantly English titles (56,858 of 56,901 titles, 99.9%, contain no Chinese characters); yearly distribution 9,970 / 10,117 / 12,002 / 11,954 / 12,858. An extended 2017–2019 pool (22,438 articles; total 79,339) serves *only* to compute advisors' pre-project publication windows and does not enter RTAS or topic modeling.

**Advisor–article linkage.** Advisor names are converted to pinyin keys (forward and reverse), matched against normalized OpenAlex author names with a college prior; ambiguous keys are skipped. 33,312 articles (58.5%) are assigned to colleges (mean 1.37 colleges per matched article); per project, advisor covariate status is matched (2,750), ambiguous (748), or unmatched (216), with unmatched covariates coded NA (not zero).
...
- **LLM second rater.** deepseek-reasoner (temperature 0, prompt v1.1), all 150 pairs blind: κ = 0.459, ρ = 0.638 (p = 1.7×10⁻¹⁸), agreement 93.3%. Triangulation against Rater B: ρ = 0.621, AUC = 0.932—above the frozen reference values, providing converging evidence; does not replace the frozen C1 estimates.
...
All reported statistics were generated from frozen analysis outputs; values are rounded for presentation. Seed 42 was used wherever stochastic procedures required a random seed.
...
**Advisor-dependence sensitivity.** The primary model clusters projects only within colleges, although advisors supervise repeated projects and 528 projects are co-supervised (multiple membership; no primary-advisor flag exists in the records). To account for within-advisor dependence without duplicating co-supervised projects, the sensitivity was restricted to the 3,186 single-advisor projects (2,716 complete cases; 1,221 advisors, 680 with ≥2 projects). A crossed random-intercept model, (1|college)+(1|advisor), converged to a boundary solution (college variance estimated at zero) and was judged unreliable: the key advisor-level covariates vary mainly *between* advisors (the within-advisor SD of log pre-project publications is 18% of its total SD, and the value is constant across projects for ~30% of repeat advisors), so advisor random intercepts and these covariates are not stably separable. The conservative alternative—college fixed effects with standard errors clustered by advisor—left every substantive conclusion unchanged: pre-project publications β = +0.0043 (p = 1.5×10⁻⁵; 95% CI [0.0023, 0.0062], excluding zero), prior supervisory load β = −0.0053 (p = 0.004; CI [−0.0088, −0.0017]), year β = +0.0026 (p = 0.001), and null funding-level indicators. A homonym guard that clustered standard errors by college × advisor name (1,333 clusters, versus 1,221 raw name clusters, so identical names across colleges are not merged) gave virtually identical standard errors and the same conclusions. Coefficient magnitudes changed only modestly relative to the frozen primary model, while coefficient signs and inferential conclusions were preserved. The substantive fixed-effect conclusions were therefore robust to advisor-clustered inference in the single-advisor subset.
...
9. **Multiple membership.** The advisor-dependence sensitivity (Sect. 3.4) covers only single-advisor projects, because 528 co-supervised projects have no primary-advisor designation; a multiple-membership random-effects model for co-supervised projects remains future work.
