...
**Submission clean version v1.0 (frozen from master manuscript v1.0-cand.10; all statistics from frozen CSVs in `03_FINAL_ANALYSIS/`)**
...
Research universities worldwide have made the integration of research and teaching a central policy goal. China's National Undergraduate Innovation and Entrepreneurship Training Program (hereafter *undergraduate innovation projects*) funds tens of thousands of student projects annually at three administrative levels—university, provincial, and national—with the explicit expectation that students engage with authentic, frontier research under faculty supervision. A natural question for scientometrics is whether the topics students actually pursue align with the research their colleges are actively publishing, and how institutional mechanisms—funding level, advisor characteristics, and the concentration of supervisory resources—relate to that alignment.
...
A single-institution design is a deliberate choice: it holds the funding regime, governance, and time window constant while spanning 40 discipline organizations—a natural laboratory for observing between-college variation—at the cost of external validity (Sect. 5).
...
**Projects.** 3,714 undergraduate innovation projects approved in 2020–2024 at the case university (university-level 1,375, 37.0%; provincial 1,657, 44.6%; national 682, 18.4%), each with title (92% Chinese), level, year, college (40), and advisor(s) (1,940 unique).
...
- **LLM second rater.** deepseek-reasoner (temperature 0, prompt v1.1), all 150 pairs blind: κ = 0.459, ρ = 0.638 (p < 10⁻¹⁸), agreement 93.3%. Triangulation against Rater B: ρ = 0.621, AUC = 0.932—above the frozen reference values, suggesting the frozen C1 is conservative rather than inflated.
- **Naive lexical baselines.** Char 3–5-gram TF-IDF cosine, char-3-gram Jaccard, and BM25 Okapi applied directly to the Chinese–English pairs (no translation alignment; hence *naive* lexical baselines, structurally disadvantaged in this cross-lingual setting) reach ρ ≈ +0.30 and AUC ≈ 0.64, below MiniLM's +0.405 / 0.880. The comparison supports multilingual semantic embedding over naive lexical overlap for the embedding choice; it makes no claim against translation-based or stronger cross-lingual methods.
- **Not benchmarked.** Newer multilingual encoders such as BGE-M3 were not benchmarked in the present study and remain an important robustness extension (Sect. 5).

### 2.5 Topic modeling

BERTopic (UMAP → HDBSCAN → c-TF-IDF) is fit on the 60,615-document joint corpus with frozen parameters (random_state = 42). HDBSCAN yields 886 non-outlier topics plus a 38.47% outlier share—documents not assigned to any stable cluster under the frozen density-based parameters; quadrant and lag analyses are conditional on the 886 clustered topics (covering 61.53% of documents). K-stability across re-runs is high (ARI = 0.93–0.98 at K = 800/850/900). Clustering-parameter sensitivity (min_cluster_size/min_samples = 15/7, 20/10, 8/3) leaves outlier rates stable (0.364–0.396) and the qualitative quadrant structure unchanged; topic counts vary with granularity (454–1,287), as expected.
...
- **RQ4.** Gini and Pareto concentration of national projects across supervisors; a supervisor-year lagged logistic model P(nat_t) ~ nat_{t−1} + log(1+load_{t−1}) + year FE (categorical year fixed effects, standard errors clustered by supervisor; main sample 736 supervisor-years, 491 supervisors; robustness sample 2,302/1,621) replaces a cross-sectional top-5% regression whose OR is constructively inflated by definition (top-5% membership is itself ranked on cumulative national projects).
...
The unconditional means model gives ICC = 0.7523; the full random-intercept model yields ICC = 0.7376—about three quarters of RTAS variance lies between colleges. The marginal R² was 0.0075, whereas the conditional R² was 0.7395, indicating that the observed fixed effects explained relatively little variance compared with the substantial between-college component. Advisor covariates dominate the project level:
...
3. **Single institution.** The natural-laboratory benefit comes at the cost of external validity; replication at universities with different disciplinary layouts is required before generalization.
4. **Observational design.** All associations (including capacity dilution) are non-causal; a directional panel design (RI-CLPM) was scoped but not estimated in this study because the planned cumulative waves were nested and the final wave lacked data; non-overlapping annual waves are required.
...
7. **Encoder scope.** Newer multilingual encoders such as BGE-M3 were not benchmarked in the present study and remain an important robustness extension; MiniLM remains the primary (frozen) encoder.
...
**Ethics approval.** The study uses administrative records and publicly retrievable bibliographic metadata; no human-subjects intervention. [Institutional review statement, if required, to be completed.]
