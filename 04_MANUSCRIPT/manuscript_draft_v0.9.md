...
> **12K→57K 升级后 RTAS 分布变化说明（对结论的影响）**：均值从 0.145 降到 0.134，标准差从 0.119 收窄到 0.072（更稳定）。这是样本扩大后的正常"均值回归"现象——前 12K 仅覆盖约 21% 武大论文，且 2020 年偏高，RTAS 估计受抽样波动影响大；57K 池全量覆盖，估计精度提高。对**结论的影响在 §8.1/§9 中同步更新**：HLM 中"国家级"这个项目等级 dummy 不再显著（旧 12K 池是假阳性的小样本显著）；**导师立项前 3 年发文量在 v1.0 数据血缘审计后仍为强显著正效应（β=+0.0043, p=3.2×10⁻⁹***，窗口 [t−3,t−1]、未匹配记 NA），同时新发现导师**立项前**累计指导项目数为显著负效应（β=−0.0063, p=1.9×10⁻⁵***，v1.0-cand.3 clean 前置口径）**——论文故事更干净也更细致：RTAS 与导师立项前的持续科研产出呈稳健正相关，与项目头衔无独立关联；较高的立项前指导负荷与更低的对齐相关（观察性关联，非因果）。
...
硬阈值 paper ≥ 1.5%、project ≥ 0.5% 是为 K ≈ 2,449 的旧 SBERT 模型设计的；对 K=307 时最热门主题 paper 流行度仅 1.32%（12K 池），或 K=886 时最热门主题 paper 流行度约 1.4%（821/56,901 = 1.443%，57K 池，与 Figure 3 横轴最大值一致），均无法满足。
...
| 象限 | 含义（全期静态流行度，非时间滞后） | 主题数 | 论文总数 | 项目总数 |
|------|------|--------|---------|---------|
| **HRHT** 高研高训 | 研究侧与项目侧流行度均高 | 5 | 2,090 | 308 |
| **HRLT** 高研低训 | 研究侧流行度高、项目侧流行度低 | **173** | 18,141 | 399 |
| **LRHT** 低研高训 | 研究侧流行度低、项目侧流行度高 | 10 | 219 | 278 |
| **LRLT** 低研低训 | 两端流行度均低 | 698 | 15,097 | 765 |
| **合计** | | **886** | **35,547** | **1,750** |

**关键发现：** HRLT 象限有 **173 个主题**（相较 12K 池 K=307 的 59 个放大了 2.9×），研究侧流行度高（18,141 篇论文）而项目侧流行度低（仅 399 个项目）——这是**全期流行度错位最集中的象限**；它是否对应"培养内容滞后于研究"的时间性解读，只能在 §6 的 defined-lag 规则下对其中可定义 lag 的主题检验（§3.4/§6：HRLT 173 个中仅 24 个可定义）。LRHT 从 12K 池 K=307 的 25 个主题缩减到 **10 个**——原来被 12K 抽样偏差夸大的低研高训象限被真实收敛；保留下来的 10 个是研究侧流行度低、项目侧流行度高的主题，值得专项分析。**重要注意**：四象限是**全期静态流行度划分**，象限标签本身不含任何时间先后含义；"研究先于/滞后于培养"的表述仅适用于 §6 中 lag 可定义的 29 个主题。LRHT 定义是 `frac_paper < 阈值（科研低，全期不足约 50 篇）`，在后续扩散滞后 §6 的**单年非平凡出现**规则下（论文端需某一单年 ≥3 篇且占该年论文 ≥0.2%，即单年约 ≥20 篇），LRHT 10 个主题在 2020–2024 内论文端无一达标（实测 0/10），lag 不可定义；LRLT 同理（0/698）。注意这是本语料中的经验事实而非逻辑恒等，详见 §6.1 口径说明。
...
>
> **稳健性对照（v2 全期指导数，含未来信息，仅作披露）**：旧 `advisor_total_supervised_projects`（2020–2024 全期计数）下指导负荷 β=−0.00654, p=1.1×10⁻⁴ ***，与 clean 版方向、量级一致——高指导负荷负关联不依赖窗口口径；正式模型以 clean 前置版为准。

**关键解读：**

1. **导师立项前科研活跃度：稳健正相关（positive association；β=+0.0043, p=3.2×10⁻⁹）**。由于协变量严格取立项前窗口 [t−3, t−1]，该关联具有清晰的时间先后方向：导师在指导该项目**之前**三年持续发表论文，其学生选题与学院科研组合的语义对齐显著更高。量级上，立项前 3 年从 0 篇增至 10 篇（log(1+x) 从 0 到 2.4）对应 RTAS 约 +0.010（约 0.14 个 RTAS 标准差）。注意：v0.9.1 版本曾报告同一构念 β=+0.0085, p=4.5×10⁻⁷，但那是脏列（3,712/3,714 为零）造成的假阳性；在重建的干净协变量上，该关联依然成立且稳健——这说明**导师科研活跃与选题对齐正相关**的结论方向未变，但效应量比旧报告更小、估计更可信。

2. **导师立项前指导负荷（prior supervisory load）：负关联（β=−0.0063, p=1.9×10⁻⁵）**：导师**立项前**累计指导项目数越多，其学生项目的 RTAS 越低。指导项目数从 1 个增至 20 个（log(1+x) 差 2.35）对应 RTAS 约 −0.015。注意协变量为严格前置口径（year < t），且项目库始于 2020，2020 届恒为 0、2021 届近似截断——这是左截断而非泄漏，估计主要来自 2022–2024 的年内变异；v1.0 旧版全期计数（含未来信息）给出几乎相同的估计（β=−0.0065），说明该结论不依赖窗口口径。与第 1 点合并的解读是：**与对齐相关的是导师本人的持续科研产出，而非其累计指导数量**——较高的立项前指导负荷（同时/连年指导大量项目）与选题对齐度下降相关。**措辞口径（v1.0-cand.8，投稿版沿用）：该关联为观察性关联，consistent with a capacity-dilution interpretation（与"容量挤占/注意力分散"解释相容），but not identified causally——未经因果识别，高指导负荷负关联仅为解释性标签而非机制结论，不能排除与高指导负荷相关的未观测导师/学院特征（如行政事务负担）。**

3. **项目等级头衔：控制学院结构后无独立关联（no independent association）**。省级 β=+0.0006 (ns)、国家级 β=−0.0016 (ns)，95% CI 均跨 0。结合 Figure 4 的 ANOVA/Tukey 结果（未控制学院与导师时 U < P ≤ N），完整的解释链是：等级间的原始差异真实存在，但纳入学院随机截距与导师特征后，**国家级/省级头衔本身与对齐度无独立关联**——差异由学院结构与导师活力承载。这与马太效应分析（§5）中"资源高度集中但 Top5% 导师 RTAS 不更高"的发现相互印证：**行政层级 ≠ 知识对齐**。
...
| HLM 国家级 β (vs 校级) | — | **−0.0016 (ns, p=0.452, CI [−0.0059,+0.0026])** | v1.0 仍 ns；行政头衔无独立关联（no independent association） |
| HLM 省级 β (vs 校级) | — | **+0.0006 (ns, p=0.720)** | v0.9.1 的边界 * (p=0.046) 在正确模型上消失 |
| HLM 导师立项前 3 年发文量 β | — | **+0.00427 (p=3.2×10⁻⁹ ***)**，log(1+x)，前置窗口 [t−3,t−1] | 稳健正相关（positive association）；Sensitivity β=+0.00478 同号同显著。v0.9.1 旧值 +0.00854 (p=4.5e-7) 为脏列假阳性，**撤回** |
| HLM 导师立项前累计指导项目数 β | — | **−0.00629 (p=1.9×10⁻⁵ ***)**，log(1+x)，严格 year < t | 新发现：立项前指导负荷（prior supervisory load）负关联；v1.0-cand.3 修复全期计数未来泄漏后结论不变（旧全期版 β=−0.00654, p=1.1×10⁻⁴） |
...
- `human_validation/150pairs_human_annotations.csv` — 150 对人工评分（annotator1_score，1–4 分；第二评分者 50 对盲评已完成，LLM 第二评分 150 对盲评已完成）
...
- `human_validation/150pairs_llm_second_rater.csv` — v1.0-cand.9：LLM 第二评分 150 对盲评结果（deepseek-reasoner, temp=0, prompt v1.1）
- `human_validation/robustness/llm_agreement_stats.csv` — v1.0-cand.9：人–LLM κ=0.459 / ρ=0.638 (p<1e-18) / 一致率 93.3%；RTAS–LLM ρ=0.279 (p=0.0005)
- `human_validation/second_rater_50pairs_filled.csv` — v1.0-cand.9：第二评分者 A 50 对盲评结果（Huang Qi）
- `human_validation/second_rater_50pairs_fanrui.csv` — v1.0-cand.9：第二评分者 B 50 对盲评结果（Fan Rui，同一 50 对）
- `human_validation/robustness/inter_rater_stats.csv` — v1.0-cand.9：评分者 A vs 参考 κ=0.465 / ρ=0.333 (p=0.018) / 一致率 72%
- `human_validation/robustness/inter_rater_stats_rater2_fanrui.csv` — v1.0-cand.9：评分者 B vs 参考 κ=0.606 / 一致率 78%（Spearman 因参考评分者极端并列失稳，以 κ 解读）；A vs B κ=0.826 / ρ=0.821 / 一致率 90%；RTAS–B 三角 ρ=0.621 (p<.001) / AUC=0.932
...
1. **公开可检索层（OpenAlex 论文记录）**：论文语料来自 OpenAlex 开放学术数据库（机构 ID `I37461747`，type=article，2020–2024，cursor 分页，§1.2）。OpenAlex 记录本身公开，任何读者可通过文档化的机构检索式完整重现 56,901 篇论文的获取。
2. **受限层（机构项目记录）**：3,714 个大学生创新创业训练计划项目记录含导师姓名等个人信息（personally identifiable information），无法公开再分发；其获取方式与字段结构在方法部分描述，不随仓库分发。
3. **衍生产出层（非识别性输出与代码）**：全部分析脚本与图表生成代码、`final_config.yaml` 等配置文件、全部正式与附录图表（PDF/PNG）、冻结的非识别性衍生数据（主题-象限汇总、聚合统计表等）统一托管于 GitHub 仓库 `RTAS-knowledge-diffusion`（https://github.com/victoryangcw/RTAS-knowledge-diffusion）。仓库内以说明文档描述数据获取与处理流程，随机种子与冻结嵌入约定（见 11.1）保证获取同等数据后可重跑一致结果。仓库当前为私有，投稿/审稿需要时转为公开。
...
| **v1.0-cand.9** | **第十七轮（单人标注验证加固，全部 secondary，冻结值零改动）：稳健化统计 AUC=0.880 [0.793,0.949]、ρ bootstrap CI [+0.281,+0.509]、置换 p<.001、LOO max\|Δρ\|=0.018；RATING_RUBRIC.md（1–4 锚点）+ 重测/第二评分者盲评表单 pre-specified 入库；LLM 第二评分（deepseek-reasoner R1, temp=0, prompt v1.1, blind）——人–LLM κ=0.459 / ρ=0.638 (p<1e-18) / 一致率 93.3%，RTAS–LLM ρ=0.279 (p=0.0005) 三角验证；第二人类评分者 50 对盲评（两名：Huang Qi κ=0.465 / ρ=0.333 (p=0.018) / 一致率 72%；Fan Rui κ=0.606 / 一致率 78%；两人互相 κ=0.826 / 一致率 90%；RTAS–FanRui 三角 ρ=0.621 (p<.001) / AUC=0.932，高于冻结值→冻结值保守）；本人重测 40 对（~12 天洗脱）——test–retest κ=0.643 / ρ=0.688 (p<.001) / 一致率 95%** | 已完成（被 v1.0-cand.10 补充取代） |
...
5. **深入"导师科研活跃 vs 项目头衔"机制的可重复性**：v1.0-cand.3 HLM（MixedLM 随机截距，§4.4）中 is_national 与 is_provincial 均为 ns（p=0.452 / 0.720），而导师**立项前** 3 年发文量 log(1+x) 为稳健正相关（positive association；β=+0.0043, p=3.2×10⁻⁹ ***），导师**立项前**累计指导项目数 log(1+x) 为负关联（negative association；β=−0.0063, p=1.9×10⁻⁵ ***；严格 year < t，2020–2021 届左截断）。与 Figure 4 Tukey HSD N−P ns、马太效应 Top5% 导师 RTAS 不更高相互印证 → 共同结论：**行政层级与资源集中并不必然伴随更高的选题对齐度；与对齐正相关的是导师立项前的持续科研产出，而较高的立项前指导负荷与对齐下降相关（consistent with a capacity-dilution interpretation；观察性关联，v1.0-cand.8 非因果口径）。** 可按导师团队规模分层、或在学院-年份面板上重复检验，并进一步区分"通讯/一作发文"与"挂名发文"的不同效应。注意 v0.9.1 旧版"近 3 年发文 β=+0.0085, p=4.5×10⁻⁷"已因脏列撤回（§4.2）。
