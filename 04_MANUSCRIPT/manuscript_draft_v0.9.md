...
- `human_validation/second_rater_50pairs_filled.csv` — v1.0-cand.9：第二评分者 A 50 对盲评结果（Rater A）
- `human_validation/second_rater_50pairs_raterB.csv` — v1.0-cand.9：第二评分者 B 50 对盲评结果（Rater B，同一 50 对）
...
| **v1.0-cand.9** | **第十七轮（单人标注验证加固，全部 secondary，冻结值零改动）：稳健化统计 AUC=0.880 [0.793,0.949]、ρ bootstrap CI [+0.281,+0.509]、置换 p<.001、LOO max\|Δρ\|=0.018；RATING_RUBRIC.md（1–4 锚点）+ 重测/第二评分者盲评表单 pre-specified 入库；LLM 第二评分（deepseek-reasoner R1, temp=0, prompt v1.1, blind）——人–LLM κ=0.459 / ρ=0.638 (p<1e-18) / 一致率 93.3%，RTAS–LLM ρ=0.279 (p=0.0005) 三角验证；第二人类评分者 50 对盲评（两名：Rater A κ=0.465 / ρ=0.333 (p=0.018) / 一致率 72%；Rater B κ=0.606 / 一致率 78%；两人互相 κ=0.826 / 一致率 90%；RTAS–Rater B 三角 ρ=0.621 (p<.001) / AUC=0.932，高于冻结值→冻结值保守）；本人重测 40 对（~12 天洗脱）——test–retest κ=0.643 / ρ=0.688 (p<.001) / 一致率 95%** | 已完成（被 v1.0-cand.10 补充取代） |
